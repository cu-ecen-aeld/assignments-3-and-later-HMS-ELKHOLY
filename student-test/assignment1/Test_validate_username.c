...
void test_validate_my_username()
{
     ...
     */
	char * hrdCoded =  my_username();
	char * conFile = malloc_username_from_conf_file();
//	printf("*****%s \n %s \n*****",hrdCoded,conFile);
	if (strcmp(hrdCoded,conFile)==0) 
		TEST_ASSERT_TRUE_MESSAGE(true,"PASS");
	else
		TEST_ASSERT_TRUE_MESSAGE(false,"FAIL");
